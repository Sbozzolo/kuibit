...
class TestGWMismatch(unittest.TestCase):
    ...
    def test__mismatch_core(self):
        ...
        pycbc_m, _ = match(ts1_pcbc, ts2_pcbc, psd=None,
        ...
        o = gwm._mismatch_core(self.ts1, self.ts2,
        ...
        o_numba = gwm._mismatch_core(self.ts1, self.ts2,
                                     fmin=fmin,
                                     fmax=fmax,
                                     noises=None,
                                     antenna_patterns=[(0, 1)],
        ...
        o_ap = gwm._mismatch_core(self.ts1, self.ts2,
                                  fmin=fmin,
                                  fmax=fmax,
                                  noises=None,
        ...
        o_noise = gwm._mismatch_core(self.ts1, self.ts2,
                                     fmin=fmin,
                                     fmax=fmax,
                                     noises=[noise],
                                     antenna_patterns=[(0, 1)],
        ...
        o_noise_fake2 = gwm._mismatch_core(self.ts1, self.ts2,
                                           fmin=fmin,
                                           fmax=fmax,
                                           noises=[noise, noise],
        ...
        o_noise_fake3 = gwm._mismatch_core(self.ts1, self.ts2,
                                           fmin=fmin,
                                           fmax=fmax,
                                           noises=[None, noise],
                                           antenna_patterns=[(0, 1), (0, 0)],
        ...
        noise_pycbc = pycbcfs.FrequencySeries(psd_noise,
                                              delta_f=df_noise)
        pycbc_m_noise, u = match(ts1_pcbc, ts2_pcbc, psd=noise_pycbc,
                                 low_frequency_cutoff=fmin,
                                 high_frequency_cutoff=fmax)

        expected_mism_noise = 1 - pycbc_m_noise

        o_noise2 = gwm._mismatch_core(self.ts1, self.ts2,
                                      fmin=fmin,
                                      fmax=fmax,
                                      noises=[noise2],
                                      antenna_patterns=[(0, 1)],
        ...
        self.assertAlmostEqual(o_noise2[0], expected_mism_noise,
                               places=2)

        # Test warning
        with self.assertWarns(Warning):
            gwm._mismatch_core(self.ts1, self.ts2,
                               fmin=fmin,
                               fmax=fmax,
                               num_polarization_shifts=50,
                               num_time_shifts=20,
    ...
    def test_mismatch(self):
        ...
        with self.assertRaises(TypeError):
            gwm.network_mismatch(self.ts1, self.ts2,
        ...
        only_virgo = gwu.Detectors(hanford=-1,
                                   livingston=-1,
                                   virgo=None)
        ...
        overlap_m = overlap(h1_pycbc, h2_pycbc, psd=None,
                            low_frequency_cutoff=fmin_gw,
                            high_frequency_cutoff=fmax_gw)

        h1_postcac = ts.TimeSeries(h1_pycbc.sample_times,
                                   hp1 - 1j * hc1)
        h2_postcac = ts.TimeSeries(h2_pycbc.sample_times,
                                   hp2 - 1j * hc2)

        o = gwm._mismatch_core(h1_postcac, h2_postcac,
                               fmin=fmin_gw,
                               fmax=fmax_gw,
                               noises=None,
        ...
        pycbc_m, _ = match(h1_pycbc, h2_pycbc, psd=None,
                           low_frequency_cutoff=fmin_gw,
                           high_frequency_cutoff=fmax_gw)

        pycbc_m = 1 - pycbc_m

        mat = gwm._mismatch_core(h1_postcac, h2_postcac,
                                 fmin=fmin_gw,
                                 fmax=fmax_gw,
                                 noises=None,
                                 antenna_patterns=[(0.66, 0.33)],
    ...
    def test_mismatch_from_psi4(self):
        ...
        self.assertAlmostEqual(gwm.network_mismatch_from_psi4(
            psi1, psi2,
            8, -70, "2015-09-14 09:50:45",
            1, 3,
            0.1,
        ...
        self.assertAlmostEqual(gwm.network_mismatch_from_psi4(
            ...
            num_polarization_shifts=30,
            num_time_shifts=30,
            time_shift_start=-70 * CU.time,
            time_shift_end=70 * CU.time)[0],
                               gwm.network_mismatch(
                                   h1, h2,
                                   8, -70, "2015-09-14 09:50:45",
                                   fmin=fmin,
