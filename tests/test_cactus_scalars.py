...
class TestCactusScalar(unittest.TestCase):
    ...
    def test_AllScalars_loads_all_supported_reductions(self):
        # Exclude non-reduction entries.
        carpet_reductions = tuple(
            reduction
            for reduction in cs.OneScalar._reduction_types.values()
            if reduction not in ("scalars", "scalar")
        )
        carpetx_reductions = tuple(
            cs.OneScalar._reduction_types_carpetx.values()
        )
        cases = [
            (
        ...
        for path, variable, reductions, header_columns in cases:
